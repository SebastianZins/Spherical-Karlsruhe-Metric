Shader "Custom/VoronoiSphereMercatorProjectionShader"
{
    SubShader
    {
            ...
            float4 applyGrid(float3 sphericalWorldPos, float4 color) 
            {
                if (_ShowGrid == 1) {
                    ...
                    if (
                        ...
                        (round(degrees(sphericalWorldPos.y) * 2) / 2) % 30 == 0 ||
                        (round(degrees(sphericalWorldPos.y) * 3) / 3) % 10 == 0
                    ) {
                        isGrid = true;
                    }
                    if (
                        (round(degrees(sphericalWorldPos.z) * 1) / 1) % 90 == 0 ||
                        (round(degrees(sphericalWorldPos.z) * 2) / 2) % 30 == 0 ||
                        (round(degrees(sphericalWorldPos.z) * 3) / 3) % 10 == 0
                    ) {
                        isGrid = true;
                    }
                    if (isGrid)
